  ...
  watch: {
    async visible(newVal) {
      if (newVal) {
        ...
        console.log(data)
        this.formData.targetActivityId = data.activityId
        this.options = [{ label: data.name, value: data.activityId }];
      }
    }
  },
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          this.loading = true
          try {
            const data = {taskId: this.task.taskId, targetActivityId: this.formData.targetActivityId}
            let response = await api.backTask(data)
